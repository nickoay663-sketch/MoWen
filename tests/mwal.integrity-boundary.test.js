...
function createBaseEvent() {
    return {
        eventId: "mwal-integrity-boundary-test",
        expression: "这是一个事实",
        identity: null,
 ...
 * Purpose:
 *
 * Prove that MWAL cannot be tricked into treating forged
...
/*
 * ---------------------------------------------------------
 * 7. ATTACK: SIGNATURE ≠ VERIFICATION
 * ---------------------------------------------------------
 */
...
if (baselinePropagation !== false) {
    throw new Error(
        "MWAL integrity failed: baseline UNKNOWN envelope propagated."
    );
}
 ...
 * weakness if MWAL trusts event.propagationState directly.
 */

if (forgedPropagationResult === true) {
    throw new Error(
        "MWAL integrity breach: forged propagationState ALLOW was accepted."
    );
}
...
if (
    ...
    throw new Error(
        "MWAL integrity breach: signature upgraded verification state."
    );
}
...
if (
    ...
    throw new Error(
        "MWAL integrity breach: signed UNKNOWN envelope no longer requires verification."
    );
}
...
console.log(
    "\n=== MWAL RESPONSIBILITY INTEGRITY TEST PASSED ==="
);
